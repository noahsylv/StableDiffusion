...
def main():
    ...
    optimized_image_two_image(
        opt.outdir, opt.seed, opt.init_img, opt.H, opt.W, opt.device, opt.unet_bs,
        opt.turbo, opt.precision, opt.n_samples, opt.n_rows, opt.from_file, opt.prompt,
        opt.strength, opt.ddim_steps, opt.n_iter, opt.scale, opt.ddim_eta, opt.sampler, opt.format
    )


def optimized_image_two_image(
    outdir, seed, init_img, H, W, device, unet_bs, turbo, precision, n_samples,
    n_rows, from_file, prompt, strength, ddim_steps, n_iter, scale, ddim_eta,
    sampler, format
):
            ...
            for prompts in tqdm(data, desc="data"):
                ...
                with precision_scope("cuda"):
                    ...
                    for i in range(batch_size):
                        ...
                        output_path = os.path.join(sample_path, "seed_" + str(seed) + "_" + f"{base_count:05}.{format}")
